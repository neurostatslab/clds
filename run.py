...
def get_args():
    ...
    parser.add_argument(
        ...
        help="Configuration file address",
        default="/",
    )
    parser.add_argument(
        "--output",
        "-o",
        metavar="Output",
        help="Folder to save the output and results",
        default="/",
    )
...
if __name__ == "__main__":
    ...
    with open(args.config, "r") as stream:
        pm = yaml.safe_load(stream)

    data_params, model_params, optim_params = (
        pm["data_params"],
        pm["model_params"],
        pm["optim_params"],
    )
    ...
    visualizations.plot_signals(
        [y[0]], titlestr="Trial", save=True, file=file + "trial"
    )
    ...
    empirical_sigmas = jnp.array(
        [EmpiricalCovariance().fit(y[:, t]).covariance_ for t in range(T)]
    )
    ...
    if hasattr(dataloader, "A"):
        visualizations.time_var_dynamics(
            ...
            titlestr=model + " (True)",
            scale=0.1 / jnp.abs(dataloader.A).max(),
            xlim=[-1, 1],
            ylim=[-0.2, 0.2],
            save=True,
            file=file + "A_true",
    ...
    visualizations.plot_states(
        [y[0], y[1]], ["y", "y"], legend=False, save=True, file=file + "states"
    )

    seed = model_params["seed"]
    key = jax.random.PRNGKey(seed)

    initial = models.InitialCondition(D, scale_tril=jnp.std(y[:, 0, 0]) * jnp.eye(D))
    ...
    emission = models.LinearEmission(
        key=k1, D=D, N=N, C=pca.components_.T, d=jnp.zeros(N)
    )
    ...
    likelihood = models.NormalConditionalLikelihood(
        N, scale_tril=model_params["obs_noise"] * jnp.eye(N)
    )
    ...
    visualizations.plot_loss(loss, save=True, file=file + "loss")

    out = {}

    EA, Eb, EL = recognition.posterior_mean(y[0], ts[0], recognition.params)
    ...
    if "b_train" in dataloader.data:
        ...
        visualizations.plot_states(
            [Eb], ["Inferred"], titlestr="$b$", save=True, file=file + "b_states"
        )
        visualizations.plot_signals(
            [
                Eb,
    ...
    if (
        "x_train" in dataloader.data
        and hasattr(dataloader, "C")
        and hasattr(dataloader, "scale_tril")
    ):
    ...
    visualizations.visualize_pc(
        mus[:, None], sigmas, linewidth=1, save=True, file=file + "pc_latent"
    )
