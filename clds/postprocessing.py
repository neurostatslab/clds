...
def identifiability_transform(params):
    ...
    """
    U, S, _ = jnp.linalg.svd(params.dynamics_cov)
...
def get_fixed_points(
    model,
    params: ParamsCLDS,
    conditions_range: Float[Array, "T M"],
    ROTATE: bool = True,
    ...
    As_conditions, _, bs_conditions, _, _ = model.weights_to_params(
        params, conditions_range
    )
    ...
    fixed_points = jax.vmap(
        lambda t: jax.scipy.linalg.solve(
            jnp.eye(model.state_dim) - As_conditions[t], bs_conditions[t]
        )
...
def compute_composite_dynamics(
    model: CLDS,
    params: ParamsCLDS,
    conditions_range: Float[Array, "N M"],
    emissions: Float[Array, "B T emission_dim"],
    conditions: Float[Array, "B T M"],
    ROTATE=True,
    n_bins=20,
    pad=1.0,
):
    ...
    def get_values_per_bin(parameters, xs):
        ...
        parameters_sum_per_bin = get_statistic_per_bin(
            jnp.array(
                [[parameters[:, i, j] for i in range(D1)] for j in range(D2)]
            ).reshape(-1, len(xs)),
            xs,
        )
        parameters_sum_per_bin = parameters_sum_per_bin.reshape(
            D1, D2, len(x_bin_edges) - 1, len(y_bin_edges) - 1
        )
    ...
    for batch_id in tqdm(range(len(emissions))):
        # Compute posterior x
        x_smooth = model.smoother(params, emissions[batch_id], conditions[batch_id])[2][
            0
        ]
        ...
        batch_As_rot = (
            jnp.einsum("ij,tjk,kl->til", H, batch_As, Hinv) if ROTATE else batch_As
        )
        ...
        thetas_sums = get_statistic_per_bin(
            conditions[batch_id], x_smooth_rot, statistic="sum"
        )
        counts = get_statistic_per_bin(
            conditions[batch_id], x_smooth_rot, statistic="count"
        )
    ...
    composite_F = jnp.zeros(
        (len(x_bin_edges) - 1, len(y_bin_edges) - 1, model.state_dim)
    )
    for x_id, _x in enumerate(x_bin_edges[:-1]):
        for y_id, _y in enumerate(y_bin_edges[:-1]):
            bin_x = jnp.array([_x, _y])
            Fx = (
                A_per_bin[:, :, x_id, y_id] - jnp.eye(model.state_dim)
            ) @ bin_x + b_per_bin[:, x_id, y_id]
