...
class Dataset:
    ...
    def project(self, x, params):
        ...
        self.C = ortho_group.rvs(
            params["N"], random_state=np.random.seed(params["seed"])
        )[:, :D]
...
class Saddle(Dataset):
    def __init__(self, params):
        ...
        a, b, c = params.get("a", -0.6), params.get("b", 2), params.get("c", -1)
        dt, time, cohs, ntrials, sigma = (
            params["dt"],
            params["time"],
            params["cohs"],
            params["ntrials"],
        ...
        for i in range(1, steps):
            ...
            rand = sigma * jax.random.normal(k1, shape=(len(cohs), ntrials, 2))
            x = x.at[:, :, i].set(
        ...
        self.data = utils.split_data_cv(
            {"y": y, "x": x, "b": b, "ts": ts}, params["props"], params["seeds"]
        )
...
class Affine(Dataset):
    def init(self, params):
        ...
        file, num_trials, sigma = (
            params["file"],
            params.get("num_trials", 200),
        ...
        for trial in range(num_trials):
            for k in range(2):
                ...
                for i in range(len(t) - 1):
                    u_ = 2 * ((k == 1) - 0.5) * inp[k] * (
                        t[i] > 0.2 and t[i] < 0.3
                    ) + 2 * ((k == 1) - 0.5) * inp[k] * (t[i] > 1.3 and t[i] < 1.4)
                    r[trial, k, i + 1] = r[trial, k, i] + (dt / tau) * r_update(
                        w, r[trial, k, i], u_, sigma
                    )
                    u[trial, k, i + 1] = u_[0]
        data["sigma"] = data
...
class RNNData(Dataset):
    def __init__(self, params):
        """Point attractor model"""
        data = jnp.load(params["file"], allow_pickle=True).item()
...
class HeadDirectionSimulation(Dataset):
    def __init__(self, params):
        ...
        # tuning curve peaks
        peaks = jnp.linspace(
            -params["tuning_peak"], params["tuning_peak"], n_neurons + 1
        )[:-1]
        ...
        def run_dynamics(key, As, bs, Cs, m0, S0):
            def f(x, inp):
                ...
                emission_noise = noise_scale * (
                    jxr.multivariate_normal(
                        subkey2, jnp.zeros(n_neurons), jnp.eye(n_neurons)
                    )
                )
